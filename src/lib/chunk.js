...
function Chunk(noa, id, i, j, k, size) {
    ...
    // vars to track if things need re-meshing
...
Chunk.prototype.updateMeshes = function () {
    if (this._terrainDirty) {
        this.noa.rendering.meshChunk(this)
        this._terrainDirty = false
    }
...
// helper class to hold the submesh data - i.e. the output of the greedy meshing algo
function Submesh(id) {
    this.id = id | 0
    this.positions = []
    this.indices = []
...
// runs the current chunk through the greedy mesher, and returns [Submesh]
Chunk.prototype.mesh = function (getMaterial, getColor, doAO, aoValues, revAoVal) {
    // create merged terrain mesh
...
var profile_hook = (function () {
    ...
    var every = 200
    var timer = new (require('./util').Timer)(every, 'Chunk meshing')
