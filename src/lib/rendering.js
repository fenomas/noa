...
function initScene(self, canvas, opts) {
  ...
  // apply some defaults
  self._light = new BABYLON.HemisphericLight(
    "light",
    new vec3(0.1, 1, 0.3),
    scene
  );
  function arrToColor(a) {
    return new col3(a[0], a[1], a[2]);
  }
...
Rendering.prototype.highlightBlockFace = function(show, posArr, normArr) {
  ...
  if (show) {
    ...
    for (var i = 0; i < 3; ++i) {
      pos[i] = Math.floor(posArr[i]) + 0.5 + (0.5 + slop) * normArr[i];
    }
...
Rendering.prototype.getCameraVector = function() {
  return vec3.TransformCoordinates(
    BABYLON.Axis.Z,
    this._rotationHolder.getWorldMatrix()
  );
};
var zero = vec3.Zero();
...
Rendering.prototype.prepareChunkForRendering = function(chunk) {
  ...
  chunk.octreeBlock = new BABYLON.OctreeBlock(
    min,
    max,
    undefined,
    undefined,
...
function updateCamera(self) {
  ...
  var id = self.noa.world.getBlockID(
    Math.floor(cam.x),
    Math.floor(cam.y),
    Math.floor(cam.z)
  );
...
function getHighlightMesh(rendering) {
  ...
  if (!m) {
    ...
    // outline
    var s = 0.5;
    var lines = BABYLON.Mesh.CreateLines(
      "hightlightLines",
      [
        new vec3(s, s, 0),
        new vec3(s, -s, 0),
