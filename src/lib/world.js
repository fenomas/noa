...
function processChunkQueues(self) {
  ...
  if (self._chunkIDsToCreate.length >= self._maxChunksPendingCreation)
    return done
  ...
  for (var di of ilocs) {
    ...
    for (var dj of jlocs) {
      ...
      for (var dk of klocs) {
        ...
        _modifyBlockData(world, i + di, j + dj, k + dk, lx, ly, lz, val)
      }
    }
  }
}
  ...
  (function() {
    ...
    profile_queues = function(world, state) {
      if (state === 'start') {
        ...
        if (world._chunkIDsToMesh.length + world._chunkIDsToMeshFirst.length)
          meshcts.push(nmesh)
        // avg queue sizes
        ...
        if (iter === every) {
          ...
          console.log(
            ...
            '- work/frame: ',
            'req',
            rnd(reqcts.reduce(sum, 0) / reqcts.length),
            'rem',
            rnd(remcts.reduce(sum, 0) / remcts.length),
            'mesh',
            rnd(meshcts.reduce(sum, 0) / meshcts.length)
          )
          iter = 0
        }
      }
    }
    var sum = function(num, prev) {
      return num + prev
    }
    var rnd = function(n) {
