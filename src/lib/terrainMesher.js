...
function MeshBuilder() {
  ...
  this.build = function(chunk, meshdata, ignoreMaterials) {
    ...
    var mats = results.matIDs.map(id =>
      getTerrainMaterial(id, ignoreMaterials)
    );
...
function GreedyMesher() {
  ...
  this.mesh = function(arr, getMaterial, getColor, doAO, aoValues, revAoVal) {
    ...
    for (var d = 0; d < 3; ++d) {
      ...
      var arrT = arr
        .transpose(d, u, v)
        .lo(1, 1, 1)
        .hi(arr.shape[d] - 2, arr.shape[u] - 2, arr.shape[v] - 2);
  ...
  function packAOMaskNoReverse(data, ipos, ineg, j, k) {
    ...
    if (data.get(ipos, j + 1, k) & solidBit) {
      ++a10;
      ++a11;
    }
    if (data.get(ipos, j - 1, k) & solidBit) {
    ...
    if (data.get(ipos, j, k + 1) & solidBit) {
      ++a01;
      ++a11;
    }
    if (data.get(ipos, j, k - 1) & solidBit) {
    ...
    if (facingSolid) {
      ...
      if (a11 === 1 && data.get(ipos, j + 1, k + 1) & solidBit) {
        a11 = 2;
      }
      if (a01 === 1 && data.get(ipos, j - 1, k + 1) & solidBit) {
        a01 = 2;
      }
      if (a10 === 1 && data.get(ipos, j + 1, k - 1) & solidBit) {
        a10 = 2;
      }
      if (a00 === 1 && data.get(ipos, j - 1, k - 1) & solidBit) {
        a00 = 2;
      }
    }
