 ...
 *    or [front, left, back, right, top, bottom]
...
Registry.prototype.registerBlock = function(id, material, properties, solid, opaque, fluid) {
    ...
    this._blockSolidity[id] = (solid == null) ? true : !!solid
    this._blockOpacity[id] = (opaque == null) ? true : !!opaque
    ...
    if (typeof mat == 'undefined') {
        ...
        mats = [mat[2], mat[2], mat[2], mat[2], mat[0], mat[1]]
    } else if (mat.length && mat.length == 6) {
        // interpret as [front, left, back, right, top, bottom]
        mats = mat
    ...
    if (this._blockIsFluid[id]) {
        ...
        if (p.fluidDensity == null) { p.fluidDensity = 1.0 }
        if (p.viscosity == null) { p.viscosity = 0.5 }
    }
