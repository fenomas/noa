...
function Entities(noa, opts) {
	...
	this.names.shadow = this.createComponent(require('../components/shadow')(noa, shadowDist, shadowOffset))
	this.names.player = this.createComponent(require('../components/player')(noa))
...
Entities.prototype.setEntitySize = function (id, xs, ys, zs) {
	...
	if (this.hasPosition(id)) {
		var pos = this.getPositionData(id)
		pos.width = (xs + zs) / 2
		pos.height = ys
		setAABBSize(pos.aabb, xs, ys, zs)
	} else if (this.hasPhysics(id)) {
		var b = this.getPhysicsBody(id)
		setAABBSize(b.aabb, xs, ys, zs)
	}
}

function setAABBSize(box, xs, ys, zs) {
	vec3.set(box.vec, xs, ys, zs)
	vec3.add(box.max, box.base, box.vec)
}


/** @param box */
Entities.prototype.getEntitiesInAABB = function (box) {
	var hits = _entsInBox
	var posArr = this.getStatesList(this.names.position)
	hits.length = 0
	for (var i = 0; i < posArr.length; i++) {
		var bb = posArr[i].aabb
		if (box.intersects(bb)) hits.push(posArr[i].__id)
	}
	return hits
}
var _entsInBox = []
