...
function updateMeshPositions(self, dt) {
	...
	for (var i = 0; i < states.length; ++i) {
		...
		vec3.scaleAndAdd(pos, pos, body.velocity, frac)
...
function doCameraTracking(self, dt) {
	...
	var cam = self.getPositionData(cid)
	var player = self.getPositionData(pid)
	
	// camera target at 90% of entity's height
	var hoff = 0.9 * player.height
	if (self.hasComponent(pid, self.components.mesh)) {
		var dat = self.getMeshData(pid)
		var pos = dat.mesh.position
		var meshYoff = dat.offset[1]
		cam.setPosition(pos.x, pos.y - meshYoff + hoff, pos.z)
	} else {
		var arr = player.position
		cam.setPosition(arr[0], arr[1] + hoff, arr[2])
	}
}
