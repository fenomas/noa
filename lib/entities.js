...
function EntityManager(noa, opts) {
  ...
  // experimental
  this.ecs = noa.ecs
  this.ecs.createComponent('shadow', { mesh:null })
  this.ecs.createComponent('position', { pos:[0,0,0] })
}
...
proto.tick = function(dt) {
  ...
  var entarr = []
  for (i=0; i<len; ++i) {
    if (this.entities[i].isSprite) entarr.push(this.entities[i])
  }
  if (entarr.length) this.noa.rendering.adjustSpriteMeshHeights(entarr)
  // call tick functions
  for (i=0; i<len; ++i) {
...
proto.updateEntitiesForRender = function(dt) {
  ...
  for (var i=0; i<this.entities.length; ++i) {
    ...
    if (this.ecs.entityHasComponent(ent.id, 'shadow')) {
    // if (ent.shadowMesh) {
      // ent.shadowMesh.position.x = ent.mesh.position.x
      // ent.shadowMesh.position.z = ent.mesh.position.z

      // this.ecs.getEntityComponentData(ent.id, 'position').pos = [ p.x, p.y, p.z ]
      var entpos = this.ecs.getEntityComponentData(ent.id, 'position').pos
      vec3.copy(entpos, pos)
      
      var shadow = this.ecs.getEntityComponentData(ent.id, 'shadow').mesh
...
proto.add = function(position, width, height, // required
  ...
  // experimental
  ent.id = this.ecs.createEntity()
  this.ecs.addEntityComponents(ent.id, ['position'])
  var pos = this.ecs.getEntityComponentData(ent.id, 'position').pos
  pos[0] = position[0]
  pos[1] = position[1]
  pos[2] = position[2]
...
function addShadow(self, ent) {
  var shadowMesh = self.noa.rendering.makeMeshInstance('shadow', false)
  
  self.ecs.addEntityComponents(ent.id, ['shadow'])
  self.ecs.getEntityComponentData(ent.id, 'shadow').mesh = shadowMesh
  
  // ent.shadowMesh = shadowMesh
}


function updateShadowHeights(self) {
  
  var shadowList = self.ecs.getComponentDataList('shadow')
  var ids = Object.keys(shadowList)
  for (var i=0; i<ids.length; ++i) {
    var id = ids[i]
    var mesh = shadowList[id].mesh
    var loc = self.ecs.getEntityComponentData(id, 'position').pos
    if (!mesh || !loc) throw new Error()
  
    // if (id==0) console.log('update:', id, self.ecs.getEntityComponentData(id, 'position').pos)
  // for (var i=0; i<self.entities.length; ++i) {
    // var e = self.entities[i]
    // if (!e.shadowMesh) continue

    // var loc = e.getPosition()
    // var mesh = e.shadowMesh
    var pick = self.noa.pick(loc, [0,-1,0], self.shadowDist)
    if (pick) {
      var y = pick.position[1]
      mesh.position.y = y + 0.05
      var scale = 1//e.bb.vec[0] // TODO: revisit for non-square entities?
      var dist = loc[1] - y
