...
function Chunk( noa, i, j, k, xs, ys, zs ) {
  // packed data storage
  var arr = new Uint16Array( xs * ys * zs )
  this.array = new ndarray( arr, [xs,ys,zs] )
  this.i = i
  this.j = j
  this.k = k
  this.shape = [xs, ys, zs]
...
Chunk.prototype.set = function( x, y, z, id ) {
  var solid  = (this._solidLookup[id])  ? SOLID_BIT  : 0
  var opaque = (this._opaqueLookup[id]) ? OPAQUE_BIT : 0
  this.array.set( x,y,z, id | solid | opaque )
}
...
Chunk.prototype.initData = function() {
  ...
  for (var i=0; i<len; ++i) {
    id = arr[i]
    s = sol[id] ? SOLID_BIT  : 0
    o = op[id]  ? OPAQUE_BIT : 0
    arr[i] = id | s | o
  }
}
 ...
 *        AO part stitched together out of spider webs and dreams
 *    
 *    Arguments:
    ...
    for( i=0; i<=len0; ++i ) {
      ...
      for(k=0; k<len2; ++k) {
        for(j=0; j<len1; ++j) {
          ...
          id0 = ( i<1     ? 0 : arrT.get(i-1, j, k))
          id1 = ( i==len0 ? 0 : arrT.get(  i, j, k))
