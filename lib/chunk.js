...
function Chunk(noa, i, j, k, size) {
    ...
    this._objectMeshLookup = noa.registry._blockMesh
    this._handlersLookup = noa.registry._blockHandlers
...
Chunk.prototype.set = function (x, y, z, id) {
    ...
    // call block handlers
    checkHandler(this, oldIDnum, false, x, y, z)
    checkHandler(this, id, true, x, y, z)

    // mark terrain dirty unless neither block was terrain
    if (isTerrain(oldID) || isTerrain(newID)) this._terrainDirty = true;
}

function checkHandler(chunk, blockID, create, x, y, z) {
    var lookup = chunk._handlersLookup[blockID]
    var handler = (create) ? lookup.onCreate : lookup.onDestroy
    if (!handler) return
    // ignore handlers if block is in chunk's edge padding blocks
    var s = chunk.size
...
Chunk.prototype.initData = function () {
    ...
    // do one scan through looking for object blocks (for later meshing) and create handlers
    ...
    for (i = 0; i < len0; ++i) {
        for (var j = 0; j < len1; ++j) {
            for (var k = 0; k < len2; ++k) {
                var packed = view.get(i, j, k)
                if (OBJECT_BIT & packed) objList.push(i, j, k)
                var h = hnd[ID_MASK & packed]
                if (h && h.onCreate) h.onCreate(this.x + i, this.y + j, this.z + k)
            }
        }
    }
...
Chunk.prototype.dispose = function () {
    // look through the data for onDestroy handlers
    var view = this._unpaddedView
    for (var i = 0; i < this.size; ++i) {
        for (var j = 0; j < this.size; ++j) {
            for (var k = 0; k < this.size; ++k) {
                var id = ID_MASK & view.get(i, j, k)
                var h = this._handlersLookup[id]
                if (h && h.onDestroy) h.onDestroy(this.x + i, this.y + j, this.z + k)
            }
        }
    }
...
function addObjectMeshAt(chunk, id, x, y, z) {
    var key = x + '|' + y + '|' + z
    var srcMesh = chunk._objectMeshLookup[id]
    ...
    var handlers = chunk._handlersLookup[id]
    if (handlers.onCustomMeshCreate) {
