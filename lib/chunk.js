...
Chunk.prototype.initData = function () {
    ...
    for (var i = 0; i < datalen; ++i) arr[i] = packID(arr[i])
...
var DEBUG = 1
if (DEBUG) var timer = new (require('./util').Timer)(500)
function debug_impl(state) {
    if (state === 'start') timer.start()
    else if (state === 'end') timer.report()
    else timer.add(state)
}
var debug_hook = (DEBUG) ? debug_impl : function () { }
