...
function addObjectMeshAt(chunk, id, x,y,z) {
  console.log('adding', id, 'at',x,y,z)
  var key = [x,y,z].join('|')
  var m = chunk.noa.rendering._makeMeshInstanceByID(id)
  m.position.x = x + chunk.i*chunk.size
  m.position.y = y + chunk.j*chunk.size
  m.position.z = z + chunk.k*chunk.size
