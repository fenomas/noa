...
function callBlockHandler(chunk, blockID, type, x, y, z) {
    ...
    var handler = lookup && lookup[type]
    if (!handler) return
...
Chunk.prototype.initData = function () {
    ...
    // do one scan through looking for object blocks (for later meshing) and onLoad handlers
    ...
    for (i = 0; i < len0; ++i) {
        for (var j = 0; j < len1; ++j) {
            for (var k = 0; k < len2; ++k) {
                ...
                var id = ID_MASK & packed
                callBlockHandler(this, id, 'onLoad', i, j, k)
            }
        }
    }
    ...
    for (var i = 0; i < this.size; ++i) {
        for (var j = 0; j < this.size; ++j) {
            for (var k = 0; k < this.size; ++k) {
                ...
                callBlockHandler(this, id, 'onUnload', i, j, k)
            }
        }
    }
