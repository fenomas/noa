...
//    _spriteMat.useAlphaFromDiffuseTexture = true
...
function doDeferredMeshing(self) {
  ...
  if (meshdata.length) {
    ...
    if (DEBUG_NO_SUBMESHES) {
      mesh = makeChunkMesh(self, meshdata, obj.id, obj.x, obj.y, obj.z);
    } else {
      mesh = makeSubmeshes(self, meshdata, obj.id, obj.x, obj.y, obj.z);
    }
