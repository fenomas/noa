...
Rendering.prototype._makeMeshInstanceByID = function(id) {
  var mesh = this.noa.registry._getMeshByBlockID(id)
  return instantiateMesh(mesh)
}

function instantiateMesh(mesh) {
...
function doDeferredMeshing(self) {
  ...
  // chunks track own dirtiness flag for terrain - skip if not set
  if (!chunk._terrainDirty) return
...
function createOrUpdateOctrees(self, mesh, chunk, x, y, z) {
  ...
  // create scene Octree if necessary
  if (!octree) {
