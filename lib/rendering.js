...
Rendering.prototype.removeDynamicMesh = function(mesh) {
  if (DEBUG_OCTREES) {
    ...
    if (i>-1) this._octree.dynamicContent.splice(i,1);
  }
}
...
function checkCameraObstructions(self) {
  ...
  if (result) {
    ...
    self._camPosOffset.copyFromFloats(off[0], off[1], off[2]).scaleInPlace(0.1)
  } else {
...
function updateCamera(self) {
  ...
  var id  = self.noa.getBlock( Math.floor(cam.x), Math.floor(cam.y), Math.floor(cam.z) )
...
function makeTerrainMaterial(self, id) {
  ...
  if (matData.alpha < 1) {
    ...
    console.log('make alpha mat', id, mat.alpha)
    window.mat = mat
  }
  return mat
}
